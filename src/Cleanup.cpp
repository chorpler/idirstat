 ...
 *   Summary:	iDirStat classes to reclaim disk space
...
const QMap<QString, QString> & Cleanup::desktopSpecificApps()
{
    ...
    if ( apps.isEmpty() )
    {
	QString desktop = QString::fromUtf8( qgetenv( "iDirStat_DESKTOP" ) );

	if ( desktop.isEmpty() )
	     desktop = QString::fromUtf8( qgetenv( "XDG_CURRENT_DESKTOP" ) );
	else
	{
	    logDebug() << "Overriding $XDG_CURRENT_DESKTOP with $iDirStat_DESKTOP (\""
		       << desktop << "\")" << endl;
	}
	...
	else
	{
		...
		// it will terminate when iDirStat terminates,

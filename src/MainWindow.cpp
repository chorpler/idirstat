...
using namespace iDirStat;

using iDirStat::DataColumns;
using iDirStat::DirTreeModel;
using iDirStat::SelectionModel;
using iDirStat::CleanupCollection;
...
void MainWindow::connectActions()
{
    ...
    CONNECT_ACTION( _ui->actionNewWindow,			    this, spawnNewWindow()	    );
...
void MainWindow::busyDisplay()
{
    ...
    int sortCol = iDirStat::DataColumns::toViewCol( iDirStat::NameCol );
...
{
    updateActions();
    int sortCol = iDirStat::DataColumns::toViewCol( iDirStat::PercentNumCol );
...
void MainWindow::openUrl( const QString & url )
{
    ...
    QString winTitle2 = "iDirStat v1.0b (tobias@familie-feigl.de - iDirStat@ Stefan.Hundhammer@gmx.de & kdirstat@ http://kdirstat.sourceforge.net)";
    QString winTitle1 = "iDirStat v1.0b";
    winTitle1 += " (";
    winTitle1 += url;
    winTitle1 += ")";
    this->setWindowTitle(winTitle1);
...
{
    int button = QMessageBox::question( this, tr( "New Window" ),
                        tr( "Launch new window?" ),
                        QMessageBox::Ok |
                        QMessageBox::Cancel );
...
//        QString appPath = QCoreApplication::applicationFilePath();
        QString appFullPath = QCoreApplication::applicationFilePath();
        int appLen = appFullPath.length();
        QString appPath = appFullPath;
        appPath.resize(appLen - 24);
        QString appFlag = "-n";
        QString appLaunch = "\"" + appPath + "\"";
...
//#endif
        ...
        logInfo() << "Trying to spawn: " << program << " " << appFlag << " " << appLaunch << endl;
        QStringList argsList = QStringList();
        logInfo() << "Args iteration 1: " << argsList << endl;
        argsList << appFlag;
        logInfo() << "Args iteration 2: " << argsList << endl;
        argsList << appLaunch;
        logInfo() << "Args iteration 3: " << argsList << endl;

//        logInfo() << "Starting " << program << " " << appFlag << " " << appLaunch << endl;
//        bool status = QProcess::startDetached( program, QStringList() << appFlag << appLaunch );
        bool status = QProcess::startDetached( program, argsList);
        logInfo() << "Spawn result: " << status << endl;
    }
...
void MainWindow::askWriteCache()
{
    QString fileName = QFileDialog::getSaveFileName( this, // parent
						     tr( "Enter name for iDirStat cache file"),
						     DEFAULT_CACHE_NAME );
    if ( ! fileName.isEmpty() )
    {
...
void MainWindow::showFileTypeStats()
{
    if ( ! _fileTypeStatsWindow )
    {
        ...
        _fileTypeStatsWindow = new iDirStat::FileTypeStatsWindow( _selectionModel,
                                                                  this );
    }
...
{
    QString helpUrl = "https://github.com/shundhammer/idirstat/blob/master/README.md";
    QString program = "/usr/bin/xdg-open";
...
{
    QString homePage = "https://github.com/shundhammer/idirstat";
    QString mailTo   = "idirstat@gmx.de";

    QString text = "<h2>iDirStat " iDirStat_VERSION "</h2>";
    text += "<p>";
    ...
    QMessageBox::about( this, tr( "About iDirStat" ), text );
}
...
void MainWindow::itemClicked( const QModelIndex & index )
{
	...
	logDebug() << "Clicked row " << index.row()
		   << " col " << index.column()
		   << " (" << iDirStat::DataColumns::fromViewCol( index.column() ) << ")"
		   << "\t" << item
		   << endl;
